...
| 2026-05-07 | 작업 효율화 — `styles.css` 4373→38줄(@import entry only). `styles/` 12개 모듈로 분할: `tokens.css`(Tier 1+2+3, 586) / `base.css`(247) / `icon.css`(18) / `responsive.css`(104) / `components/{landing,toolbar,sidebar,inbox,preview-card,side-panel,automation,overlay}.css`. CSS 내용 0 변경(순수 cut/paste). Load order: tokens → base → icon → components/* → responsive. Tier 3 토큰이 원본 후반(L557~818)에서 `tokens.css` 상단으로 이동했지만 `:root --var`는 lexical resolution이라 cascade 무관. 트리거: 단일 4373줄 파일이 sub-agent 작업 시 토큰/응답 지연 야기. frontend-dev 담당 파일 범위에 `styles/**/*.css` 추가. file:// 호환 유지. |
| 2026-05-07 | 작업 효율화 — `.claude/SPEC_MAP.md` 신규 (84줄). spec.md §N(11개) / §N-M(51개) 라인 범위 표 + Read `offset`/`limit` 사용 예. CLAUDE.md spec 참조 정책 + `code-reviewer` / `frontend-dev` / `mock-data-writer` 3개 sub-agent 정의의 "§0 → grep → 라인 계산" 절차를 "SPEC_MAP 진입점 → 즉시 offset/limit Read"로 단축. spec.md 본문 무수정(§0 self-doc은 redundant 표시로 보존). 갱신 정책: spec 큰 섹션 추가/이동 시 동기 갱신, 본문 ±50줄 누적 시 갱신. spec.md 분할은 91개 cross-reference + §10 working log 컨벤션 깨짐 우려로 비채택, 대신 zero-risk additive 인덱스 도입. |
