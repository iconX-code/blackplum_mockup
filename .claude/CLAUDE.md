...
- **spec 참조 정책**: spec.md / req.md는 1800+ / 580+ 줄로 통째로 Read 시 컨텍스트 폭발(60k+) → 응답 사이클 지연 + watchdog stall 사고 패턴(실제 발생 이력). main thread는 sub-agent 위임 시 가능하면 `.claude/SPEC_MAP.md`(라인 범위 표)를 진입점으로 안내하거나 정확한 섹션 위치를 핀포인트로 지정(예: "spec.md §4-3 / 라인 919~949만 Read"). 또는 schema/토큰 등 핵심 정보를 위임 프롬프트에 inline 제공하여 agent의 spec 재읽기를 차단. 자세한 정책은 `.claude/SPEC_MAP.md` / sub-agent 정의 참조
- **Serena MCP 활용 정책 (.js 파일 한정)**: 본 프로젝트의 메인 코드(`index.html`)는 `<script type="text/babel">` JSX-in-HTML 구조라 TypeScript LSP가 파싱하지 못해 Serena의 symbol 도구가 작동하지 않는다. 따라서 **Serena는 순수 .js 파일(`mock-data.js` / `icon-set.js` / `eslint.config.js`) 작업 시에만 사용**하고, 그 외 모든 파일(`index.html` / `styles.css` / `spec.md` / `req.md` / `tasks.md` / `CLAUDE.md` / `.claude/**`)은 built-in `Read` / `Grep` / `Edit`로 처리한다. 추가로 Serena 도구는 deferred로 등록돼 schema가 미로드 상태이므로, 위 .js 파일 작업 직전에 `ToolSearch(query="select:mcp__serena__find_symbol,mcp__serena__find_referencing_symbols,mcp__serena__get_symbols_overview,mcp__serena__replace_symbol_body,mcp__serena__insert_after_symbol,mcp__serena__search_for_pattern")`로 schema를 일괄 로드한 뒤 호출한다. 첫 세션의 `activate_project` 호출 시 typescript-language-server 자동 다운로드/인덱싱으로 수십 초~1분 소요(1회성, 이후 캐시됨). 이 정책은 sub-agent에도 동일하게 적용 — `mock-data-writer`(mock-data.js 담당)와 `frontend-dev`(icon-set.js 담당)는 Serena 우선, `index.html`/`styles.css` 작업분은 built-in 사용
